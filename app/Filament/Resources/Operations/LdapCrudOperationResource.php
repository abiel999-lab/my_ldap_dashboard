...
KeyValue::make('objectclass_must_values')
                ...
                Section::make('4. Preview / Execution Result')
                    ...
                    ->schema([
                        Textarea::make('preview_result')
                            ->label('Preview Result JSON')
                            ->formatStateUsing(fn ($state): string => is_array($state) ? json_encode($state, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) : (string) ($state ?? ''))
                            ->disabled()
                            ->rows(12)
                            ->columnSpanFull(),

                        Textarea::make('execution_result')
                            ->label('Apply Result JSON')
                            ->formatStateUsing(fn ($state): string => is_array($state) ? json_encode($state, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) : (string) ($state ?? ''))
                            ->disabled()
                            ->rows(12)
                            ->columnSpanFull(),

                        Textarea::make('rollback_payload')
                            ->label('Rollback Payload JSON')
                            ->formatStateUsing(fn ($state): string => is_array($state) ? json_encode($state, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) : (string) ($state ?? ''))
                            ->disabled()
                            ->rows(12)
                            ->columnSpanFull(),

                        Textarea::make('rollback_result')
                            ->label('Rollback Result JSON')
                            ->formatStateUsing(fn ($state): string => is_array($state) ? json_encode($state, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) : (string) ($state ?? ''))
                            ->disabled()
                            ->rows(12)
                            ->columnSpanFull(),
