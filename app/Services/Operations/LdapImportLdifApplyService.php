...
class LdapImportLdifApplyService
{
    ...
    private function applyRawLdifWithCommand(string $ldif): void
    {
        ...
        if (str_contains($lower, 'dn: cn=schema,cn=config')) {
            $encoded = base64_encode($ldif);
            $namespace = env('PETRA_IAM_KUBERNETES_NAMESPACE', 'petra-iam');

            $inner = "cat > /tmp/schema-import.b64 <<'EOF'\n"
                . $encoded
                . "\nEOF\n"
                . "base64 -d /tmp/schema-import.b64 > /tmp/schema-import.ldif\n"
                . "ldapmodify -Y EXTERNAL -H ldapi:/// -f /tmp/schema-import.ldif";

            $cmd = 'microk8s kubectl -n '.escapeshellarg($namespace)
                .' exec deploy/openldap -- bash -lc '.escapeshellarg($inner).' 2>&1';

            exec($cmd, $output, $code);

            if ($code !== 0) {
                throw new \RuntimeException('schema ldapmodify failed: '.implode("\n", $output));
            }

            return;
        }

        $tmp = tempnam(sys_get_temp_dir(), 'ldap-import-');

        if ($tmp === false) {
            throw new \RuntimeException('Cannot create temporary LDIF file.');
        }
