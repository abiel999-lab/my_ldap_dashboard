...
class LdapSchemaAttributeCatalogService
{
    ...
    public function defaultConnectionId(): ?int
    {
        ...
        if (! $connection) {
            $connection = LdapConnection::query()->orderBy('id')->first();
        }

        return $connection?->id;
    }

    public function structuralObjectClassOptionsForConnectionWithFallback(?int $connectionId): array
    {
        if (! $connectionId) {
        ...
        if (! $connectionId) {
            return $this->fallbackStructuralObjectClasses();
        }

        $options = $this->structuralObjectClassOptionsForConnection((int) $connectionId);
    ...
    public function auxiliaryObjectClassOptionsForConnectionWithFallback(?int $connectionId): array
    {
        if (! $connectionId) {
            $connectionId = $this->defaultConnectionId();
        }
        ...
        if (! $connectionId) {
            return $this->fallbackAuxiliaryObjectClasses();
        }

        $options = $this->auxiliaryObjectClassOptionsForConnection((int) $connectionId);
    ...
    private function fallbackStructuralObjectClasses(): array
    {
        return [
            'inetOrgPerson' => 'inetOrgPerson — STRUCTURAL — MUST: cn, sn — common user account',
    ...
    private function fallbackAuxiliaryObjectClasses(): array
    {
        return [
