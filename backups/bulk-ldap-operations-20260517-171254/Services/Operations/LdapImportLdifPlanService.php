...
class LdapImportLdifPlanService
{
    ...
    public function buildRowLdif(string $operation, string $dn, array $payload): array
    {
        ...
        return match ($operation) {
            ...
            'rename_dn', 'rename', 'modrdn' => $this->buildModRdnLdif($dn, $payload, false),
            ...
            'schema_create', 'schema_add' => $this->buildSchemaCreateLdif($payload),
            'schema_modify', 'schema_update' => $this->buildSchemaModifyLdif($payload),
            'schema_delete' => $this->buildSchemaDeleteLdif($payload),
    ...
    private function buildSchemaModifyLdif(array $payload): array
    {
        $dn = $this->normalizeDn(trim((string) (
            $payload['dn']
            ?? $payload['DN']
            ?? $payload['schema_dn']
            ?? $payload['schemaDn']
        ...
        if ($dn === '') {
            $schemaName = trim((string) ($payload['schema'] ?? $payload['schemaName'] ?? $payload['schema_name'] ?? 'custom'));
            $schemaName = preg_replace('/[^a-zA-Z0-9_-]+/', '-', $schemaName) ?: 'custom';
            $dn = 'cn='.$schemaName.',cn=schema,cn=config';
        }

        $operation = strtolower(trim((string) (
            $payload['schema_operation']
            ?? $payload['modify_operation']
            ?? $payload['operation']
            ?? 'add'
        )));

        if (! in_array($operation, ['add', 'replace', 'delete'], true)) {
            $operation = 'add';
        }

        $attribute = trim((string) (
            $payload['attribute']
            ?? $payload['schema_attribute']
            ?? $payload['schemaAttribute']
            ?? ''
        ));
        ...
        /*
         * Shortcut from structured definition.
         */
        if ($attribute === '') {
            $type = strtolower(trim((string) ($payload['type'] ?? '')));

            if (in_array($type, ['attributetype', 'attribute_type', 'attribute'], true)) {
                $attribute = 'olcAttributeTypes';
                $value = $this->buildOlcAttributeType($payload);
            } elseif (in_array($type, ['objectclass', 'object_class'], true)) {
                $attribute = 'olcObjectClasses';
                $value = $this->buildOlcObjectClass($payload);
            }
        }

        if ($attribute === '') {
            return $this->invalidResult('schema_modify', 'modify', $dn, 'Schema modify requires attribute or type.');
        }

        $ldif = [];
        $ldif[] = 'dn: '.$this->escapeLdifValue($dn);
        $ldif[] = 'changetype: modify';
        $ldif[] = $operation.': '.$attribute;
        ...
        $reverse = implode(PHP_EOL, [
            '# Schema rollback is dangerous and disabled/manual by default.',
            'dn: '.$this->escapeLdifValue($dn),
            '# changetype: modify',
        ]);
        ...
        return [
            ...
            'changetype' => 'modify',
            'dn' => $dn,
            'message' => 'LDIF schema modify generated.',
            'generated_ldif' => $generated,
            'reverse_ldif' => $reverse,
        ];
    }

    private function buildSchemaDeleteLdif(array $payload): array
    {
        $dn = $this->normalizeDn(trim((string) (
            $payload['dn']
            ?? $payload['DN']
        ...
        if ($dn === '') {
            $schemaName = trim((string) ($payload['schema'] ?? $payload['schemaName'] ?? $payload['schema_name'] ?? ''));
        ...
        if ($dn === '') {
            return $this->invalidResult('schema_delete', 'delete', '', 'Schema delete requires dn or schema name.');
        }

        $generated = implode(PHP_EOL, [
            '# WARNING: Schema delete is dangerous. Ensure schema is not used by existing entries.',
            'dn: '.$this->escapeLdifValue($dn),
            'changetype: delete',
        ]);

        $reverse = implode(PHP_EOL, [
            '# Schema delete rollback requires full old schema snapshot.',
            '# Manual rollback only.',
            'dn: '.$this->escapeLdifValue($dn),
            '# changetype: add',
        ]);
        ...
        return [
            ...
            'changetype' => 'delete',
            'dn' => $dn,
            'message' => 'LDIF schema delete generated. WARNING: schema delete is dangerous.',
            'generated_ldif' => $generated,
            'reverse_ldif' => $reverse,
        ];
    }
