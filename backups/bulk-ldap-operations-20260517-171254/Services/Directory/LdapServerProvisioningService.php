    ...
    public function refreshGeneratedArtifacts(LdapServer $server): LdapServer
    {
        try {
            ...
            $this->logLdapServerActivity(
                server: $server,
                ok: true,
                action: 'refresh_generated_artifacts',
                message: 'LDAP server generated artifacts refreshed.',
                extra: [
                    'artifacts' => [
                        'docker_command' => true,
                        'docker_compose_yaml' => true,
            ...
            $this->logLdapServerActivity(
                ...
                ok: false,
                action: 'refresh_generated_artifacts',
                message: 'Failed to refresh LDAP server generated artifacts: '.$exception->getMessage(),
                extra: [
...
YAML;
    ...
    public function testConnection(LdapServer $server): array
    {
        ...
        if (! function_exists('ldap_connect')) {
            ...
            $this->logLdapServerActivity(
                server: $server,
                ok: false,
                action: 'service_test_connection',
    ...
    public function registerAsLdapConnection(LdapServer $server): array
    {
        ...
        $payload = [
            ...
            'bind_password' => Crypt::encryptString((string) $server->admin_password),
            ...
            'schema_bind_password' => Crypt::encryptString((string) $server->admin_password),
    ...
    public function startDockerContainer(LdapServer $server): array
    {
        ...
        if (! $exists['ok']) {
            $this->logDockerResult($server, 'service_start_docker_container', $exists, [
                'phase' => 'check_existing_containers',
                'container' => $container,
            ]);
        ...
        if (in_array($container, $names, true)) {
            ...
            $this->logDockerResult($server, 'service_start_docker_container', $start, [
                'phase' => 'start_existing_container',
                'container' => $container,
            ]);
        ...
        $this->logDockerResult($server, 'service_start_docker_container', $run, [
            'phase' => 'run_new_container',
            'container' => $container,
            'image' => $image,
            'port' => $port,
        ]);
    ...
    public function stopDockerContainer(LdapServer $server): array
    {
        ...
        $this->logDockerResult($server, 'service_stop_docker_container', $result, [
            'container' => $container,
        ]);
    ...
    public function restartDockerContainer(LdapServer $server): array
    {
        ...
        $this->logDockerResult($server, 'service_restart_docker_container', $result, [
            'container' => $container,
        ]);
    ...
    public function checkDockerContainer(LdapServer $server): array
    {
        ...
        if ($result['ok']) {
            ...
            $this->logDockerResult($server, 'service_check_docker_container', $final, [
                'container' => $container,
        ...
        $this->logDockerResult($server, 'service_check_docker_container', $result, [
            'container' => $container,
        ]);
    ...
    private function logDockerResult(LdapServer $server, string $action, array $result, array $extra = []): void
    {
        $this->logLdapServerActivity(
            server: $server,
            ok: (bool) ($result['ok'] ?? false),
            action: $action,
            message: (string) ($result['message'] ?? 'Docker action completed.'),
            extra: array_merge([
                'result' => $result,
            ], $extra),
        );
    ...
    private function logLdapServerActivity(
        ...
        try {
            $context = array_merge([
                ...
                'target_id' => $server->getKey(),
                'target_label' => $server->name ?? $server->server_name ?? null,
                'host' => $server->host ?? $server->hostname ?? $server->url ?? null,
                'port' => $server->ldap_port ?? $server->port ?? null,
                'base_dn' => $server->base_dn ?? null,
                'container_name' => $server->container_name ?? null,
                'docker_image' => $server->docker_image ?? null,
                'provision_mode' => $server->provision_mode ?? null,
                'expose_mode' => $server->expose_mode ?? null,
                'source' => 'service',
                'total' => 1,
                'success' => $ok ? 1 : 0,
                'failed' => $ok ? 0 : 1,
