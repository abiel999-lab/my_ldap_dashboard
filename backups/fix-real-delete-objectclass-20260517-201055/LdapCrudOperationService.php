...
class LdapCrudOperationService
{
    ...
    private function previewAddObjectClass(LdapCrudOperation $operation, array $entry, string $dn): array
    {
        ...
        return [
            ...
            'rollback_plan' => [
                'remove_added_objectClass' => $objectClass,
                'remove_added_attributes' => array_keys((array) $operation->objectclass_must_values),
            ],
        ];
    }
    ...
    private function previewDeleteObjectClass(LdapCrudOperation $operation, array $entry, string $dn): array
    {
        ...
        $attributes = app(LdapSchemaDropdownService::class)
        ...
        return [
            ...
            'status' => 'planned',
            'planned_action' => 'delete_objectclass',
            'reason' => 'Akan menghapus auxiliary objectClass ' . $objectClass . ' dan related attributes jika aman.',
            'current_objectclasses' => $entry['objectclass'] ?? [],
            'before' => [
    ...
    private function previewAddAttribute(LdapCrudOperation $operation, array $entry, string $dn): array
    {
        ...
        if ($value === '') {
            return [
                ...
                'status' => 'blocked',
                'planned_action' => 'add_attribute',
                'reason' => 'Attribute value kosong.',
            ];
        }

        $key = strtolower($attribute);
        $existingValues = $entry[$key] ?? [];
        $exists = ! empty($existingValues);
        ...
        return [
            ...
            'planned_action' => 'add_attribute',
            'reason' => $exists
                ? 'Attribute sudah ada. Behavior: ' . $behavior
                : 'Attribute belum ada dan akan ditambahkan.',
            'before' => [
            ...
            'rollback_plan' => [
                'restore_old_values' => $existingValues,
                'remove_added_value' => $value,
            ],
        ];
    }
    ...
    private function previewDeleteAttribute(LdapCrudOperation $operation, array $entry, string $dn): array
    {
        ...
        if ($this->isMustAttributeOnCurrentEntry($operation, $entry, $attribute)) {
            return [
                'dn' => $dn,
                'status' => 'blocked',
                'planned_action' => 'delete_attribute',
                'reason' => 'Attribute adalah MUST attribute dari objectClass aktif, tidak aman dihapus.',
                'attribute' => $attribute,
                'existing_values' => $existingValues,
            ];
        }
    ...
    private function previewMoveOu(LdapCrudOperation $operation, array $entry, string $dn): array
    {
        ...
        return [
            ...
            'before' => [
                'old_dn' => $dn,
                'old_parent_dn' => $oldParent,
            ],
            'planned_changes' => [
                'rdn' => $rdn,
                'new_parent_dn' => $targetOuDn,
                'new_dn' => $newDn,
            ],
            'rollback_plan' => [
                'move_back_to_parent_dn' => $oldParent,
                'original_dn' => $dn,
            ],
        ];
    }
    ...
    private function previewDeleteEntry(LdapCrudOperation $operation, array $entry, string $dn): array
    {
        return [
            ...
            'reason' => 'Entry akan dihapus. Pastikan target benar. Recursive delete belum disarankan.',
            'before' => [
                'dn' => $dn,
            ...
            'rollback_plan' => [
                'restore_entry_from_snapshot' => true,
                'snapshot' => $entry,
            ],
    ...
    private function isMustAttributeOnCurrentEntry(LdapCrudOperation $operation, array $entry, string $attribute): bool
    {
        $objectClasses = $entry['objectclass'] ?? [];

        foreach ($objectClasses as $objectClass) {
            $must = app(LdapSchemaDropdownService::class)
                ->mustAttributes($operation->ldap_connection_id, $objectClass);

            foreach ($must as $mustAttribute) {
                if (strtolower((string) $mustAttribute) === strtolower($attribute)) {
                    return true;
                }
            }
        }
