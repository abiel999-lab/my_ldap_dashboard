...
class LdapCrudOperationService
{
    ...
    private function relatedAttributeDeletePlan(LdapCrudOperation $operation): array
    {
        if ($operation->operation_kind !== 'delete_objectclass') {
            return [];
        }
        ...
        $attributes = app(LdapSchemaDropdownService::class)
            ->attributeOptions($operation->ldap_connection_id, $operation->objectclass_name);

        return [
            'objectclass' => $operation->objectclass_name,
            'delete_related_attributes' => true,
            'must_attributes' => $must,
            'candidate_attributes' => array_values($attributes),
            'rule' => 'Saat Delete ObjectClass, attribute terkait objectClass ikut direncanakan delete jika aman. Attribute yang masih dibutuhkan objectClass lain harus di-skip.',
        ];
    }
