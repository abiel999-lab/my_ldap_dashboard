...
class LdapSchemaEntryResource extends Resource
{
    ...
    {
        try {
            $ldapConnectionId = isset($data['ldap_connection_id']) && $data['ldap_connection_id']
                ? (int) $data['ldap_connection_id']
                : null;

            $reset = array_key_exists('reset', $data)
                ? (bool) $data['reset']
                : true;

            $row = [
                'uuid' => (string) \Illuminate\Support\Str::uuid(),
                'command_type' => 'ldap_schema_sync_queued',
                'command' => 'queued job: SyncLdapSchemaEntriesJob',
                'status' => 'running',
                'is_safe_mode' => true,
                'safe_mode' => true,
                'is_preview' => false,
            ...
            $columns = \Illuminate\Support\Facades\Schema::getColumnListing('command_executions');

            $row = collect($row)
                ->filter(fn ($value, string $column): bool => in_array($column, $columns, true))
                ->toArray();

            $executionId = \Illuminate\Support\Facades\DB::table('command_executions')->insertGetId($row);

            SyncLdapSchemaEntriesJob::dispatch($ldapConnectionId, $reset, $executionId);

            Notification::make()
                ->title('LDAP schema sync queued')
                ->body('Schema sync masuk queue. Command Execution ID: '.$executionId)
                ->success()
                ->send();
        } catch (Throwable $e) {
            Notification::make()
