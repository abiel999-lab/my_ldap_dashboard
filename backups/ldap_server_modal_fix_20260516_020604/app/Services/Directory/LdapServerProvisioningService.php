...
YAML;
    ...
    public function registerAsLdapConnection(LdapServer $server): array
    {
        ...
        $payload = [
            ...
            'ldap_port' => $server->ldap_port,
            'scheme' => $server->scheme,
            'base_dn' => $server->base_dn,
            'bind_dn' => $server->admin_dn,
            'username' => $server->admin_dn,
            'user_dn' => $server->admin_dn,
            'bind_password' => $server->admin_password,
            'password' => $server->admin_password,
            'bind_pass' => $server->admin_password,
            'is_active' => true,
            'is_default' => false,
            'status' => 'active',
            'description' => 'Generated from LDAP Server Provisioning #'.$server->id,
            'created_at' => now(),
            'updated_at' => now(),
        ];
        ...
        if ($insert === []) {
            return [
                'ok' => false,
                'message' => 'No compatible ldap_connections columns found.',
            ];
        }

        $existing = null;

        if (Schema::hasColumn($table, 'host') && Schema::hasColumn($table, 'port')) {
        ...
        if (! $existing && Schema::hasColumn($table, 'ldap_host') && Schema::hasColumn($table, 'ldap_port')) {
            $existing = DB::table($table)
                ->where('ldap_host', $server->host)
                ->where('ldap_port', $server->ldap_port)
                ->first();
        }

        if ($existing) {
            DB::table($table)->where('id', $existing->id)->update(array_merge($insert, [
                'updated_at' => now(),
            ]));
        ...
        return [
            'ok' => true,
            'message' => 'LDAP Connection registered.',
        ];
    }
