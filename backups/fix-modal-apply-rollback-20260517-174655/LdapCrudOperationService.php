...
class LdapCrudOperationService
{
    ...
    public function runDryRun(LdapCrudOperation $operation): array
    {
        ...
        foreach ($entries as $entry) {
            $row = [
                'dn' => $entry['dn'] ?? null,
                'status' => $useQueue ? 'queued_safe_mode' : 'dry_run_success',
                'reason' => $useQueue
                    ? 'Jumlah entry besar. Seharusnya diproses via Laravel Queue Job.'
                    : 'Dry-run sukses. LDAP asli belum diubah.',
        ...
        $result = [
            'ok' => true,
            'message' => $useQueue
                ? 'Dry-run selesai. Entry besar ditandai untuk queue.'
                : 'Dry-run selesai. LDAP asli belum diubah.',
            'used_queue' => $useQueue,
            'results' => $results,
        ];
        ...
        $operation->forceFill([
            ...
            'rollback_payload' => [
                'operation_kind' => $operation->operation_kind,
                'generated_from' => 'dry_run',
                'items' => $results,
            ],
            'executed_at' => now(),
        ])->save();

        $this->writeAudit($operation, 'dry_run', 'success', $result['message']);
        $this->writeOperationLog($operation, 'dry_run', 'success', $result['message']);
    ...
    public function rollback(LdapCrudOperation $operation): array
    {
        if (empty($operation->rollback_payload)) {
            return $this->fail('Rollback payload belum tersedia. Jalankan dry-run / real apply dulu.');
        }

        $result = [
            'ok' => true,
            'message' => 'Rollback safe mode selesai. LDAP asli belum diubah.',
    ...
    private function fakePreviewEntries(LdapCrudOperation $operation, string $baseDn): array
    {
        if (($operation->target_mode ?? 'base_dn') === 'custom_dn') {
            return [[
                ...
                'reason' => 'Custom Target DN akan diproses.',
            ]];
        }
        ...
        return [[
            ...
            'status' => 'planned',
            'planned_action' => $operation->operation_kind,
            'reason' => 'Contoh planned entry. Nanti diganti hasil LDAP search asli.',
        ]];
    }
