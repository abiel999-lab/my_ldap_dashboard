                ...
                Section::make('Import File')
                    ->schema([
                        ...
                        Select::make('import_type')
                            ...
                            ->disabled()
                            ->dehydrated()
                            ->helperText('Auto-detected from uploaded file extension: .csv, .json, .ldif.')
                            ->required(),

                        Select::make('ldap_connection_id')
                            ->label('LDAP Connection')
                            ->options(fn (): array => [0 => 'All LDAP - Preview Only'] + LdapConnection::query()
                            ...
                            ->searchable()
                            ->preload()
                            ->dehydrateStateUsing(fn ($state): ?int => ((int) $state === 0 ? null : (int) $state))
                            ->helperText('All LDAP is for preview/check only. Real apply requires one target LDAP connection.')
                        ...
                        FileUpload::make('file_path')
                            ...
                            ->afterStateUpdated(function (Set $set, $state): void {
                                ...
                                $set('import_type', $type);

                                if ($path !== '') {
                                    $set('original_filename', basename($path));
                                }
                            })
                            ...
                            ->required()
                            ->helperText('Upload CSV, LDIF, or JSON. Import Type will be auto-detected. Preview will not change LDAP data.'),
