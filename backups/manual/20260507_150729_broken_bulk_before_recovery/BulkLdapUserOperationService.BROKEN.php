...
class BulkLdapUserOperationService
{
    ...
    private function hasBulkExecutionStarted(BulkLdapOperation $operation): bool
    {
        $operation->refresh();

        if (in_array($operation->status, ['queued', 'running', 'success', 'partial_success', 'failed'], true)) {
            return true;
        }

        if ((int) $operation->processed_items > 0) {
            return true;
        }

        if ($operation->items()
            ->whereIn('status', ['running', 'success', 'failed', 'already_applied', 'conflict', 'skipped'])
            ->exists()
        ) {
            return true;
        }
    ...
    private function isNormalQueueApplyAllowed(BulkLdapOperation $operation): bool
    {
        $operation->refresh();
    ...
    private function isRetryFailedAllowed(BulkLdapOperation $operation): bool
    {
        $operation->refresh();

        if (! in_array($operation->status, ['failed', 'partial_success'], true)) {
            return false;
        }

        return $operation->items()
            ->whereIn('status', ['failed', 'pending', 'running'])
            ->exists();
    }
    ...
    public function previewCreateTestUsers(BulkLdapOperation $operation): array
    {
        ...
        if ($this->hasBulkExecutionStarted($operation)) {
            return [
                'ok' => false,
                'message' => 'This bulk operation has already started or finished. For safety, create a new bulk operation instead of regenerating preview.',
            ];
        }

        if ($operation->operation_type === 'bulk_delete_test_users') {
            return $this->previewDeleteTestUsers($operation);
        }
    ...
    public function previewDeleteTestUsers(BulkLdapOperation $operation): array
    {
        $operation->refresh();
        ...
        if ($this->hasBulkExecutionStarted($operation)) {
            return [
                'ok' => false,
                'message' => 'This bulk delete operation has already started or finished. For safety, create a new bulk delete operation instead of regenerating preview.',
            ];
        }
        ...
        DB::transaction(function () use ($operation, $targetOu, $count, $startNumber, $padding): void {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $ldif = 'dn: '.$targetDn.PHP_EOL.'changetype: delete'.PHP_EOL;
                ...
                BulkLdapOperationItem::query()->create([
                    ...
                    'metadata' => [
                        ...
                        'generated' => true,
                        'idempotent' => true,
                        'delete_guard' => 'Only deletes generated uid prefix users inside target OU.',
                    ],
                ]);
            }

            $operation->forceFill([
                'operation_type' => 'bulk_delete_test_users',
                'status' => 'previewed',
                'target_ou_dn' => $targetOu,
                'safe_mode' => true,
                'dry_run' => false,
                ...
                'finished_at' => null,
                'failed_at' => null,
                'message' => 'Bulk LDAP delete test users preview generated.',
                'error_message' => null,
        ...
        $this->audit([
            'module' => 'operations.bulk_ldap',
            'action' => 'preview_bulk_delete_test_users',
            'status' => 'success',
            'target_type' => BulkLdapOperation::class,
            'target_key' => (string) $operation->id,
            'target_dn' => $targetOu,
        ...
        return [
            'ok' => true,
            'message' => 'Delete preview generated: '.$count.' users.',
            'total_items' => $count,
        ];
    }
    ...
    public function queueApply(BulkLdapOperation $operation, bool $retryOnlyFailed = false): array
    {
        ...
        if ($retryOnlyFailed) {
            if (! $this->isRetryFailedAllowed($operation)) {
                return [
                    'ok' => false,
                    'message' => 'Retry is only allowed for failed or partial_success operations with failed/pending items.',
                ];
            }
        } else {
            if (! $this->isNormalQueueApplyAllowed($operation)) {
                return [
                    'ok' => false,
                    'message' => 'Queue Apply rejected for safety. This operation is not in a fresh previewed state or it has already started. Create a new bulk operation, or use Retry Failed if applicable.',
                ];
            }
        }
    ...
    public function executeQueuedOperation(BulkLdapOperation $operation, bool $retryOnlyFailed = false): void
    {
        ...
        $this->createBulkQueueDiagnosticLog($operation->fresh(), 'Queue worker started executing Bulk LDAP operation.', [
            'bulk_ldap_operation_id' => $operation->id,
        ...
        $this->createBulkQueueDiagnosticLog($operation->fresh(), 'Queue worker finished Bulk LDAP operation.', [
            'bulk_ldap_operation_id' => $operation->id,
            'operation_job_id' => $operation->operation_job_id,
            'final_status' => $status,
    ...
    private function executeItem(BulkLdapOperation $operation, BulkLdapOperationItem $item): void
    {
        ...
        if ($item->action === 'delete_user') {
            $this->executeDeleteItem($operation, $item);
    ...
    private function executeDeleteItem(BulkLdapOperation $operation, BulkLdapOperationItem $item): void
    {
        $startedAt = microtime(true);
        ...
        if (! $this->dnExists($operation->ldapConnection, $item->target_dn)) {
            $item->forceFill([
                'status' => 'already_applied',
                'stdout' => 'DN does not exist. Delete is already applied safely.',
                'stderr' => null,
                'exit_code' => 0,
                'finished_at' => now(),
            ])->save();
            ...
            $this->updateOperationJobItem($operation, $item, [
                ...
                'finished_at' => now(),
                'output_payload' => [
                    'message' => 'DN does not exist. Delete is already applied safely.',
                    'target_dn' => $item->target_dn,
                ],
            ]);

            $this->auditItem($operation, $item, 'already_applied', 'DN does not exist. Delete skipped safely.', null);

            return;
        }

        $command = $this->buildLdapDeleteCommand($operation->ldapConnection, $item->target_dn);
        $displayCommand = $this->displayLdapDeleteCommand($operation->ldapConnection, $item->target_dn);
        ...
        try {
            ...
            $execution->forceFill([
                ...
                'stderr' => $stderr,
                'exit_code' => $exitCode,
                'error_message' => $ok ? null : 'Bulk LDAP delete item command failed.',
                'finished_at' => now(),
                'duration_ms' => $this->durationMs($startedAt),
            ])->save();
            ...
            if ($ok) {
                ...
                $this->auditItem($operation, $item, 'success', 'Bulk LDAP item deleted successfully.', $execution);

                return;
            }

            $this->markItemFailed($operation, $item, $stderr ?: 'Bulk LDAP delete item command failed.', $execution, $stdout, $exitCode);
        } catch (Throwable $exception) {
            $execution->forceFill([
                'status' => 'failed',
                'stderr' => $this->redactString($exception->getMessage()),
    ...
    private function buildLdapDeleteCommand(LdapConnection $connection, string $targetDn): array
    {
        return [
            'ldapdelete',
            '-v',
    ...
    private function displayLdapDeleteCommand(LdapConnection $connection, string $targetDn): string
    {
        return 'ldapdelete -v -x'
    ...
    private function createCommandExecution(BulkLdapOperation $operation, BulkLdapOperationItem $item, string $displayCommand): CommandExecution
    {
        ...
        return CommandExecution::query()->create([
            ...
            'module' => 'operations.bulk_ldap',
            'command_type' => $item->action === 'delete_user' ? 'bulk_ldap_delete_user' : 'bulk_ldap_create_user',
            'status' => 'running',
    ...
    private function createOperationJobLog(
        BulkLdapOperation $operation,
        string $level,
        string $event,
        string $message,
        ...
        try {
            ...
            $payload = $this->filterTablePayload('operation_job_logs', $payload);

            if (empty($payload)) {
                return;
            }

            DB::table('operation_job_logs')->insert($payload);
        } catch (Throwable $exception) {
            return;
        }
    }

    private function createBulkQueueDiagnosticLog(BulkLdapOperation $operation, string $message, array $context = []): void
    {
        $this->createOperationJobLog(
            $operation,
            'info',
            'bulk_ldap_queue_diagnostic',
            $message,
            $context
        );
    }
    ...
    private function createOperationJob(BulkLdapOperation $operation, bool $retryOnlyFailed): ?int
    {
        ...
        try {
            ...
            // OperationJob is useful, but queue dispatch must not die just because this table schema differs.
            return null;
        }
    }
    ...
    private function updateOperationJobItem(BulkLdapOperation $operation, BulkLdapOperationItem $item, array $payload): void
    {
        /*
         * IMPORTANT:
         * Bulk LDAP Items are the source of truth for per-user progress.
         * Operation Job Items are only the generic Operations mirror.
         *
         * Some older operation_jobs schemas may fail to create an operation_job row,
         * which means $operation->operation_job_id can be null.
         * If we insert operation_job_items with null operation_job_id, PostgreSQL fails
         * and the whole queue job crashes before processing LDAP.
         *
         * Therefore: when operation_job_id is missing, skip the generic mirror.
         * Bulk LDAP Items, Command Executions, Audit Logs, and the Bulk summary still work.
         */
        if (! $operation->operation_job_id) {
            return;
        }
        ...
        try {
            if (! $item->operation_job_item_id) {
                ...
                $insert = $this->filterTablePayload('operation_job_items', $insert);

                if (empty($insert)) {
                    return;
                }

                $id = DB::table('operation_job_items')->insertGetId($insert);
            ...
            // Generic Operation Job Items mirror must never crash the LDAP bulk worker.
            return;
        }
    }
