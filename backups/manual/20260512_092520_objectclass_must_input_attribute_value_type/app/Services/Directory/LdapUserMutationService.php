...
class LdapUserMutationService
{
    ...
    public function addObjectClass(LdapUserEntry $user, string $objectClass): array
    {
        $objectClass = $this->cleanObjectClassName($objectClass);

        [$valid, $message] = app(LdapSchemaAttributeCatalogService::class)->validateAddObjectClass($user, $objectClass);
    ...
    public function removeObjectClass(LdapUserEntry $user, string $objectClass): array
    {
        ...
        if (! $valid) {
            return $this->failed($message ?? 'Remove objectClass validation failed.');
        }
        ...
        return $result + [
            'message' => 'ObjectClass removed successfully.',
        ];
    }
