                ...
                ->form(function (): array {
                    $fields = [
                        Select::make('objectClass')
                            ...
                            ->required()
                            ->live()
                            ->helperText('If the selected objectClass has missing MUST attributes, fill them below.'),
                    ...
                    $schema = app(LdapSchemaAttributeCatalogService::class)->schemaForUser($this->record);
                    $mustAttributeNames = [];

                    foreach ($schema['object_classes'] ?? [] as $objectClass) {
                        foreach ($objectClass['must'] ?? [] as $attribute) {
                            $mustAttributeNames[$attribute] = true;
                        }
                    }

                    foreach (array_keys($mustAttributeNames) as $attribute) {
                        $fields[] = TagsInput::make('must_values.'.$attribute)
                            ->label('MUST: '.$attribute)
                            ->placeholder('Type value then press Enter')
                            ->visible(function ($get) use ($attribute): bool {
                                $objectClass = (string) $get('objectClass');
                                ...
                                return array_key_exists($attribute, $missingMust);
                            })
                            ->helperText(function ($get) use ($attribute): string {
                                $objectClass = (string) $get('objectClass');
                                ...
                                $missingMust = app(LdapSchemaAttributeCatalogService::class)->missingMustAttributesForObjectClass($this->record, $objectClass);
                                $meta = $missingMust[$attribute] ?? null;

                                if (! $meta) {
                                    return '';
                                }

                                return $this->formatAttributeMeta($meta);
                            });
                    }
