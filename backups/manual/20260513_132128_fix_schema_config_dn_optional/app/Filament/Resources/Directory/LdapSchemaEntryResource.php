...
class LdapSchemaEntryResource extends Resource
{
    ...
    public static function queueSchemaMutation(string $operation, array $data, $record = null): void
    {
        try {
            ...
            if ($schemaConfigDn === '') {
                throw new \RuntimeException('Schema Config DN wajib diisi.');
            }
    ...
    public static function runDirectSchemaSync(): void
    {
        try {
            ...
            Notification::make()
                ->title('LDAP schema sync completed')
                ->body('Schema Browser sudah disinkronkan dari LDAP connection aktif.')
                ->success()
                ->send();
        } catch (Throwable $e) {
            Notification::make()
