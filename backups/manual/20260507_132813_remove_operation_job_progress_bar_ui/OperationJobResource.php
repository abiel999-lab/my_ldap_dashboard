    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('progress_bar_text')
                    ->label('Progress')
                    ->state(fn ($record): string => $record->progress_bar_text ?? '░░░░░░░░░░ 0%')
                    ->description(fn ($record): string => $record->progress_label ?? '0%')
                    ->sortable(query: function ($query, string $direction) {
                        return $query->orderBy('progress_percent', $direction);
                    }),
