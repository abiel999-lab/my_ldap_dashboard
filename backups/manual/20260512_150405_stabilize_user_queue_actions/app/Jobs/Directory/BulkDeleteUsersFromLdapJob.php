    ...
    public function handle(LdapUserLifecycleService $service): void
    {
        ...
        try {
            LdapUserEntry::query()
                ...
                ->orderBy('id')
                ->chunkById(50, function ($users) use ($service, &$ok, &$failed, &$results): void {
            ...
            $execution?->update([
                ...
                'stderr' => $failed > 0 ? 'Some users failed to delete. See stdout JSON results.' : null,
                'error_message' => $failed > 0 ? 'Partial delete failure.' : null,
                'finished_at' => now(),
            ]);
        } catch (Throwable $e) {
            $execution?->update([
