...
use App\Console\Commands\IamSyncLdapSchemaDirectCommand;
...
class ModifyLdapSchemaDefinitionJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if ($process->isSuccessful()) {
                ...
                // Sync ulang setelah schema berubah.
                try {
                    ...
                    // Jangan gagal operasi utama hanya karena resync gagal.
                    report($syncError);
                }
    ...
    private function markSuccess(array $payload): void
    {
        ...
        try {
            ...
            // Logger version mismatch should not break LDAP operation.
        }
    }
    ...
    private function markFailed(string $message, array $payload): void
    {
        ...
        try {
            if (method_exists(SafeCommandExecutionLogger::class, 'markFailed')) {
                SafeCommandExecutionLogger::markFailed($this->commandExecutionId, $message, $payload);
            }
        } catch (Throwable) {
            // Logger version mismatch should not break LDAP operation.
        }
