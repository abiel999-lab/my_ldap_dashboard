    ...
    public static function infolist(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Section::make('Resolved Group Membership')
                    ->description('Read-only relationship view generated from cached LDAP users and groups.')
                    ->schema([
                        TextEntry::make('resolved_group_count')
                            ->label('Resolved Group Count')
                            ->state(fn (LdapUserEntry $record): int => app(LdapMembershipResolver::class)->groupsForUserCount($record)),

                        TextEntry::make('resolved_groups_text')
                            ->label('Resolved Groups')
                            ->state(fn (LdapUserEntry $record): string => app(LdapMembershipResolver::class)->groupsForUserText($record))
                            ->columnSpanFull(),
                    ])
                    ->columns(2),
