...
class LdapApplyDryRunVerifier
{
    public function verify(ImportApplyPlan $plan): CommandExecution
    {
        ...
        try {
            ...
            $execution->forceFill([
                ...
                'error_message' => $process->isSuccessful() ? null : 'ldapadd dry-run verification exited with non-zero status.',
                'finished_at' => now(),
                'duration_ms' => $this->durationMs($startedAt),
            ])->save();
    ...
    private function validatePlan(ImportApplyPlan $plan): array
    {
        ...
        return [
            'ok' => true,
            'message' => 'Plan is safe for ldapadd dry-run verification.',
        ];
    }
    ...
    private function summarizeOutput(string $stdout, string $stderr): string
    {
        ...
        return 'ldapadd dry-run completed. stdout lines: '.$stdoutLines.', stderr lines: '.$stderrLines.'. LDAP data was not changed.';
    }
