...
class ImportApplyPlanService
{
    public function generate(ImportBatch $batch, ?ImportApplyPlan $plan = null): array
    {
        ...
        try {
            ...
            $plan->forceFill([
                ...
                'dry_run_verified_at' => null,
                'dry_run_verified_by' => null,
                'dry_run_command_execution_id' => null,
                'dry_run_output_summary' => null,
                'dry_run_error_message' => null,
                'real_apply_started_at' => null,
                'real_apply_finished_at' => null,
                'real_apply_by' => null,
                'real_apply_command_execution_id' => null,
                'real_apply_output_summary' => null,
                'real_apply_error_message' => null,
                'real_apply_confirmation' => null,
                ...
                'metadata' => [
                    ...
                    'schema_fix' => 'inetOrgPerson entries receive sn automatically when missing.',
                ],
                'finished_at' => now(),
            ])->save();
            ...
            app(AuditLogger::class)->log([
                ...
                'request_payload' => [
                    ...
                    'safe_mode' => true,
                    'dry_run' => true,
                    'schema_fix' => 'auto_sn_for_inetOrgPerson',
                ],
                'after_value' => [
    ...
    private function buildLdif($rows): string
    {
        ...
        $lines[] = '# Schema guard: inetOrgPerson requires sn. Missing sn is derived from cn.';
        $lines[] = '';
        ...
        foreach ($rows as $row) {
            ...
            $objectClasses = $mapped['object_classes'] ?? ($attributes['objectClass'] ?? $attributes['objectclass'] ?? []);
            ...
            $hasInetOrgPerson = collect($objectClasses)
                ->contains(fn (string $value): bool => strtolower($value) === 'inetorgperson');

            if ($hasInetOrgPerson) {
                $requiredClasses = ['top', 'person', 'organizationalPerson', 'inetOrgPerson'];

                foreach ($requiredClasses as $requiredClass) {
                    if (! collect($objectClasses)->contains(fn (string $value): bool => strtolower($value) === strtolower($requiredClass))) {
                        $objectClasses[] = $requiredClass;
                    }
                }
    ...
    {
        $cn = $attributes['cn'] ?? $attributes['displayName'] ?? $attributes['uid'] ?? 'Unknown';
