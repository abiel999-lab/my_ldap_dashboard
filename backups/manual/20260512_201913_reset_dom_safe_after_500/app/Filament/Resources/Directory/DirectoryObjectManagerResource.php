...
class DirectoryObjectManagerResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->filters([
                ...
                Tables\Filters\Filter::make('hide_deleted')
                    ...
                    ->query(fn ($query) => $query->where(fn ($query) => $query
                        ->whereNull('status')
                        ->orWhereNotIn('status', ['missing_from_ldap', 'deleted_from_ldap']))),
            ...
            ->actions([
                TableAction::make('openObject')
                    ->label(fn ($record): string => static::isUserObject($record) ? 'Open User' : 'View')
                    ->icon(fn ($record): string => static::isUserObject($record) ? 'heroicon-o-user-circle' : 'heroicon-o-eye')
                    ->color(fn ($record): string => static::isUserObject($record) ? 'info' : 'gray')
                    ->url(fn ($record): string => static::manageUrl($record)),
                    ...
                    Action::make('addObjectClass')
                        ...
                        ->form([
                            Select::make('object_class')
                                ->label('ObjectClass')
                                ->options(fn ($record): array => app(\App\Support\Directory\LdapSchemaObjectClassHelper::class)
            ...
            ->recordUrl(fn ($record): string => static::manageUrl($record))
            ->bulkActions([
        ...
        return [
            ...
            TextInput::make('rdn_value')
                ...
                ->afterStateUpdated(function (Get $get, Set $set, $state): void {
                    static::refreshMustAttributeRows($get, $set);
                }),

            Select::make('structural_object_class')
                ->label('Structural ObjectClass')
                ->options(fn (Get $get): array => app(LdapSchemaObjectClassHelper::class)
                    ->structuralOptions((int) ($get('ldap_connection_id') ?: 0)))
                ->searchable()
                ->preload()
                ->live()
                ->required()
            ...
            Select::make('auxiliary_object_classes')
                ...
                ->multiple()
                ->options(fn (Get $get): array => app(LdapSchemaObjectClassHelper::class)
                    ->auxiliaryOptions((int) ($get('ldap_connection_id') ?: 0)))
                ->searchable()
                ->preload()
                ->live()
                ->afterStateUpdated(function (Get $get, Set $set): void {
            ...
            KeyValue::make('attributes')
                ...
                ->keyLabel('Attribute')
                ->valueLabel('Value')
                ->helperText('Isi attribute LDAP biasa saja. Jangan isi objectClass di sini karena sudah diatur lewat Structural dan Auxiliary ObjectClasses. Multi-value pisahkan dengan koma.'),
        ];
    }

    public static function refreshMustAttributeRows(Get $get, Set $set): void
    {
        $connectionId = (int) ($get('ldap_connection_id') ?: 0);
        $structural = $get('structural_object_class');
        $auxiliary = (array) ($get('auxiliary_object_classes') ?: []);
        $rdnAttribute = trim((string) ($get('rdn_attribute') ?: ''));
        $rdnValue = trim((string) ($get('rdn_value') ?: ''));

        $objectClasses = array_values(array_filter(array_merge(
            ['top'],
            [$structural],
            $auxiliary,
        )));

        $must = app(LdapSchemaObjectClassHelper::class)
            ->mustAttributesForObjectClasses($connectionId, $objectClasses);

        $current = (array) ($get('attributes') ?: []);
        $next = [];

        foreach ($must as $attribute => $label) {
            if ($attribute === '') {
                continue;
            }

            // objectClass is managed by Structural ObjectClass and Auxiliary ObjectClasses fields.
            // Never allow it from the generic LDAP Attributes table.
            if (strtolower($attribute) === 'objectclass') {
                continue;
            }

            if (array_key_exists($attribute, $current)) {
                $next[$attribute] = $current[$attribute];
                continue;
            }

            if ($rdnAttribute !== '' && strtolower($attribute) === strtolower($rdnAttribute) && $rdnValue !== '') {
                $next[$attribute] = $rdnValue;
                continue;
            }
    ...
    public static function queueSyncSingleObject($record): void
    {
        try {
            ...
            \App\Jobs\Directory\SyncDirectoryObjectsJob::dispatch(
    ...
    public static function queueCreateObject(array $data): void
    {
        try {
            ...
            // objectClass must not come from LDAP Attributes table.
            ...
            // Safe auto-fill for common MUST attributes so create form is easier.
            // This is not hardcode for business logic; this is fallback convenience only.
    ...
    public static function manageUrl($record): string
    {
        if (static::isUserObject($record)) {
            $userId = static::findUserIdForDirectoryObject($record);

            if ($userId) {
                return url('/admin/directory/ldap-user-entries/'.$userId);
            }
        }
