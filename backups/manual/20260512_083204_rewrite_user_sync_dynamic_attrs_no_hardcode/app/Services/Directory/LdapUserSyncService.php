...
private function parseLdapSearchOutput(string $output): array
    {
        $entries = [];
        $current = [];
        $lastAttribute = null;
        ...
        foreach ($lines as $rawLine) {
            ...
            if (str_starts_with($rawLine, ' ') && $lastAttribute !== null) {
                $continuation = substr($rawLine, 1);
                $lastIndex = count($current[$lastAttribute]) - 1;

                if ($lastIndex >= 0) {
                    $current[$lastAttribute][$lastIndex] .= $continuation;
                }
...
private function normalizeEntry(array $entry): array
    {
        ...
        foreach ($entry as $name => $value) {
            ...
            $allAttributes[$name] = $this->asArray($value);
        }
        ...
        foreach ($allAttributes as $name => $values) {
            if ($this->isOperationalAttributeName($name)) {
                $operationalAttributes[$name] = $values;
            } else {
                $normalAttributes[$name] = $values;
            }
        }

        ksort($normalAttributes, SORT_NATURAL | SORT_FLAG_CASE);
        ksort($operationalAttributes, SORT_NATURAL | SORT_FLAG_CASE);

        $objectClasses = $this->asArray(
            $normalAttributes['objectClass']
            ?? $normalAttributes['objectclass']
            ?? []
        );

        $memberOf = $this->asArray(
            $operationalAttributes['memberOf']
            ?? $operationalAttributes['memberof']
            ?? []
        );

        $accountStatus = $this->firstValue(
            $normalAttributes['petraAccountStatus']
            ?? $normalAttributes['petraaccountstatus']
            ?? null
        );

        return [
            'entry_uuid' => $this->firstValue(
                $operationalAttributes['entryUUID']
                ?? $operationalAttributes['entryuuid']
                ?? $normalAttributes['entryUUID']
                ?? $normalAttributes['entryuuid']
                ?? null
            ),

            'uid' => $this->firstValue($normalAttributes['uid'] ?? null),
            'cn' => $this->firstValue($normalAttributes['cn'] ?? null),
            'sn' => $this->firstValue($normalAttributes['sn'] ?? null),
            'given_name' => $this->firstValue($normalAttributes['givenName'] ?? $normalAttributes['givenname'] ?? null),
            'display_name' => $this->firstValue($normalAttributes['displayName'] ?? $normalAttributes['displayname'] ?? null),
            'mail' => $this->firstValue($normalAttributes['mail'] ?? null),
            'employee_number' => $this->firstValue($normalAttributes['employeeNumber'] ?? $normalAttributes['employeenumber'] ?? null),
            'employee_type' => $this->firstValue($normalAttributes['employeeType'] ?? $normalAttributes['employeetype'] ?? null),
            ...
            'is_locked' => filled($operationalAttributes['pwdAccountLockedTime'] ?? $operationalAttributes['pwdaccountlockedtime'] ?? null),
