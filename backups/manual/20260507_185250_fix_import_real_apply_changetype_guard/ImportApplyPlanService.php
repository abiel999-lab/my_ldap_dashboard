...
class ImportApplyPlanService
{
    ...
    private function validateBatch(ImportBatch $batch): array
    {
        ...
        if (! $batch->ldap_connection_id) {
            $errors[] = 'LDAP connection is required before generating apply plan. All LDAP mode is preview-only. Edit the import batch and choose one target LDAP connection before apply.';
        }
