...
class DirectoryObjectManagerResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ActionGroup::make([
                    Action::make('viewObjectDetail')
                        ->label('View Object Detail')
                        ->icon('heroicon-o-eye')
                        ->color('gray')
                        ->visible(fn ($record): bool => ! static::isUserObject($record))
                        ->modalHeading(fn ($record): string => 'Directory Object Detail: '.static::safeRdn((string) ($record->dn ?? '')))
                        ->modalSubmitAction(false)
                        ->modalCancelActionLabel('Close')
                        ->modalWidth('7xl')
                        ->modalContent(fn ($record): HtmlString => static::objectDetailModalContent($record)),
                    ...
                    Action::make('openUser')
                        ->label('Open User Page')
                        ->icon('heroicon-o-user-circle')
                        ->color('info')
                        ->visible(fn ($record): bool => static::isUserObject($record))
                        ->url(fn ($record): string => static::userUrl($record)),
                    ...
                    Action::make('addObjectClass')
                        ...
                        ->form([
                            Select::make('object_class')
                                ->label('ObjectClass')
                                ->options(fn ($record): array => app(LdapSchemaObjectClassHelper::class)
                                    ->objectClassOptions((int) ($record->ldap_connection_id ?? 0)))
                                ->searchable()
                                ->preload()
                                ->required(),
                    ...
                    Action::make('removeObjectClass')
                        ...
                        ->form([
                            Select::make('object_class')
                                ->label('ObjectClass')
                                ->options(fn ($record): array => collect(static::extractObjectClasses($record))
    ...
    public static function queueSyncObjects(array $data): void
    {
        try {
            ...
            Notification::make()
                ->title('Directory object sync failed')
                ->body($e->getMessage())
                ->danger()
                ->send();
        }
    ...
    public static function commonMustAttributes(string $structural, array $objectClasses = []): array
    {
        ...
        foreach ($objectClasses as $objectClass) {
            ...
            foreach (($map[$objectClass] ?? []) as $attribute) {
                $must[] = $attribute;
            }
        }
    ...
    public static function isUserObject($record): bool
    {
        ...
        return in_array('inetorgperson', $classes, true)
            || str_contains($dn, ',ou=people,');
    }
    ...
    public static function userUrl($record): string
    {
        ...
        if ($id) {
            return url('/admin/directory/ldap-user-entries/'.$id);
        }

        return url('/admin/directory/ldap-user-entries');
    }
    ...
    public static function findUserIdForDirectoryObject($record): ?int
    {
        try {
            ...
            $query = \App\Models\Directory\LdapUserEntry::query()
                ->where('dn', (string) ($record->dn ?? ''));
    ...
    public static function objectDetailModalContent($record): HtmlString
    {
        $dn = (string) ($record->dn ?? 'N/A');
        $rdn = static::safeRdn($dn);
        $parentDn = static::parentDn($dn);
        $ldapConnection = static::connectionName($record->ldap_connection_id ?? null);
        $status = (string) ($record->status ?? 'active');
        $objectClasses = static::extractObjectClasses($record);

        $attributes = static::extractNormalAttributesForModal($record);
        $operationalAttributes = static::extractOperationalAttributesForModal($record);

        $html = '<div class="space-y-6 text-sm">';

        $html .= '<div class="rounded-xl border border-gray-700/60 bg-gray-900/40 p-4">';
        $html .= '<h3 class="mb-3 text-base font-semibold text-white">1. Object Overview</h3>';
        $html .= '<div class="grid grid-cols-1 gap-3 md:grid-cols-2">';
        $html .= static::detailBox('DN', $dn);
        $html .= static::detailBox('RDN', $rdn);
        $html .= static::detailBox('Parent DN', $parentDn ?: 'N/A');
        $html .= static::detailBox('LDAP Connection', $ldapConnection);
        $html .= static::detailBox('Status', $status);
        $html .= static::detailBox('ObjectClass Count', (string) count($objectClasses));
        $html .= '</div>';
        $html .= '</div>';

        $html .= '<div class="rounded-xl border border-gray-700/60 bg-gray-900/40 p-4">';
        $html .= '<h3 class="mb-3 text-base font-semibold text-white">2. Object Classes</h3>';

        if ($objectClasses === []) {
            $html .= '<p class="text-gray-400">No objectClass found.</p>';
        } else {
            $html .= '<div class="flex flex-wrap gap-2">';
            foreach ($objectClasses as $objectClass) {
                $html .= '<span class="rounded-md border border-blue-500/40 bg-blue-500/10 px-2 py-1 text-xs font-medium text-blue-200">'
                    .e((string) $objectClass).
                '</span>';
            }
            $html .= '</div>';
        }
        ...
        $html .= '<div class="rounded-xl border border-gray-700/60 bg-gray-900/40 p-4">';
        $html .= '<h3 class="mb-3 text-base font-semibold text-white">3. Directory Attributes</h3>';
        $html .= static::attributeTableHtml($attributes);
        $html .= '</div>';

        $html .= '<div class="rounded-xl border border-gray-700/60 bg-gray-900/40 p-4">';
        $html .= '<h3 class="mb-3 text-base font-semibold text-white">4. Operational / Read-only Attributes</h3>';
        $html .= static::attributeTableHtml($operationalAttributes);
        $html .= '</div>';
    ...
    public static function detailBox(string $label, string $value): string
    {
        return '<div class="rounded-lg border border-gray-700/60 bg-gray-950/30 p-3">'
            .'<div class="mb-1 text-xs font-semibold uppercase tracking-wide text-gray-400">'.e($label).'</div>'
            .'<div class="break-words font-mono text-xs text-gray-100">'.e($value).'</div>'
            .'</div>';
    }

    public static function attributeTableHtml(array $attributes): string
    {
        if ($attributes === []) {
            return '<p class="text-gray-400">No attributes found.</p>';
        }

        ksort($attributes);

        $html = '<div class="overflow-x-auto">';
        $html .= '<table class="min-w-full divide-y divide-gray-700 text-left text-xs">';
        $html .= '<thead>';
        $html .= '<tr class="text-gray-300">';
        $html .= '<th class="whitespace-nowrap px-3 py-2 font-semibold">Attribute</th>';
        $html .= '<th class="whitespace-nowrap px-3 py-2 font-semibold">Count</th>';
        $html .= '<th class="whitespace-nowrap px-3 py-2 font-semibold">Values</th>';
        $html .= '</tr>';
        $html .= '</thead>';
        $html .= '<tbody class="divide-y divide-gray-800">';
        ...
        foreach ($attributes as $attribute => $value) {
            ...
            $html .= '<tr>';
            $html .= '<td class="whitespace-nowrap px-3 py-2 align-top font-mono font-semibold text-gray-100">'.e((string) $attribute).'</td>';
            $html .= '<td class="whitespace-nowrap px-3 py-2 align-top text-gray-300">'.count($values).'</td>';
            $html .= '<td class="px-3 py-2 align-top">';

            if ($values === []) {
                $html .= '<span class="text-gray-500">empty</span>';
            } else {
                $html .= '<ul class="list-disc space-y-1 pl-4">';
                foreach ($values as $item) {
                    $html .= '<li class="break-all font-mono text-gray-200">'.e($item).'</li>';
                }
                $html .= '</ul>';
            }
    ...
    public static function extractNormalAttributesForModal($record): array
    {
        ...
        $operationalKeys = static::operationalAttributeKeys();
    ...
    public static function extractOperationalAttributesForModal($record): array
    {
