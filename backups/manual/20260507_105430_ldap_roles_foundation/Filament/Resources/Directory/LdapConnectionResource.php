    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->recordActions([
                ...
                Action::make('searchTest')
                    ->label('Search Test')
                    ->icon('heroicon-o-magnifying-glass')
                    ->color('gray')
                    ->requiresConfirmation()
                    ->modalHeading('Run LDAP search test?')
                    ->modalDescription('This will bind to LDAP and run a safe read-only search on the configured Base DN. It will not modify LDAP data.')
                    ->action(function (LdapConnection $record): void {
                        $before = $record->toArray();

                        $result = app(LdapConnectionHealthService::class)->searchTest(
                            connection: $record,
                            baseDn: $record->base_dn,
                            filter: '(objectClass=*)',
                            attributes: ['dn'],
                            limit: 20,
                        );
                        ...
                        app(AuditLogger::class)->log([
                            ...
                            'target_key' => (string) $record->getKey(),
                            'ldap_connection_id' => $record->id,
                            ...
                            'before_value' => $before,
                            'after_value' => $record->fresh()?->toArray(),
                            'error_message' => $result['ok'] ? null : $result['message'],
                            'duration_ms' => $result['duration_ms'],
                        ]);

                        Notification::make()
                            ->title($result['ok'] ? 'LDAP search succeeded' : 'LDAP search failed')
                            ->body($result['message'])
                            ->{$result['ok'] ? 'success' : 'danger'}()
                            ->send();
                    }),

                Action::make('schemaTest')
                    ->label('Schema Test')
                    ->icon('heroicon-o-code-bracket-square')
                    ->color('gray')
                    ->requiresConfirmation()
                    ->modalHeading('Run LDAP schema test?')
                    ->modalDescription('This will read RootDSE and subschema information from LDAP. It is read-only and will not modify LDAP data.')
                    ->action(function (LdapConnection $record): void {
                        $before = $record->toArray();

                        $result = app(LdapConnectionHealthService::class)->schemaTest($record);
                        ...
                        Notification::make()
                            ->title($result['ok'] ? 'LDAP schema test succeeded' : 'LDAP schema test failed')
                            ->body($result['message'])
                            ->{$result['ok'] ? 'success' : 'danger'}()
                            ->send();
                    }),
